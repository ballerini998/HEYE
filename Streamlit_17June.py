...
UI = {
    "it": {
        ...
        "schema_intro": "Questa versione mostra solo gli indicatori costruibili con i dataset gia preparati: popolazione attuale, proiezioni, disoccupazione, inattivita, migrazioni, immatricolati universitari e BesT Lombardia.",
        "data_methods_intro": "I dati restano separati per fonte e tema. La dashboard legge file processed armonizzati, mentre i file raw rimangono disponibili per controllo e aggiornamenti.",
        ...
        "main_territories": "Bergamo / Lombardia / Italia",
        ...
        "data_quality_text": "Per i tassi non vengono creati totali Maschi/Femmine senza denominatori. Per le migrazioni e gli immatricolati, invece, i totali sono calcolati solo quando i dati sono conteggi additivi. I dati BesT sono estratti dal PDF Lombardia 2025 in formato lungo.",
        "status_ok": "OK",
    ...
    "en": {
        ...
        "schema_intro": "This version shows only indicators that can be built from the datasets already prepared: current population, projections, unemployment, inactivity, migration, university first-year entrants and BesT Lombardy.",
        "data_methods_intro": "Data remain separated by source and theme. The dashboard reads harmonized processed files, while raw files remain available for checks and updates.",
        ...
        "main_territories": "Bergamo / Lombardy / Italy",
        ...
        "data_quality_text": "For rates, Male/Female totals are not computed without denominators. For migration and university entrants, totals are computed only when data are additive counts. BesT data are extracted from the Lombardy 2025 PDF in long format.",
        "status_ok": "OK",
...
INDICATORS = [
    ...
    },
    {
        ...
        "description_it": "Trend degli immatricolati per anno accademico, area/ateneo e sesso. Il totale per sesso e calcolato per somma dei conteggi F/M.",
        "description_en": "Trend in first-year university entrants by academic year, university/area and sex. The sex total is computed by summing Female/Male counts.",
        "source": "MUR USTAT / dataset immatricolati fornito",
    ...
    },
    {
        ...
        "description_it": "Indicatori BesT estratti dal report Lombardia 2025, selezionabili per dominio e indicatore. Il confronto principale mostra Bergamo, Lombardia e Italia.",
        "description_en": "BesT indicators extracted from the 2025 Lombardy report, selectable by domain and indicator. The main comparison shows Bergamo, Lombardy and Italy.",
        "source": "ISTAT BesT Lombardia 2025",
...
def filter_real_data(df, ind, lang):
    ...
    if ind["chart"] == "bes_best_indicator":
        ...
        if scope == text("main_territories", lang):
            df = df[df["main_comparison"].astype(str).str.lower().isin(["true", "1"])]
        return df
...
def render_indicator(ind, population_df, projection_df, unemployment_df, inactivity_df, migration_df, university_df, bes_df, lang):
    ...
    with meta3:
        st.markdown(f"**{text('data_status', lang)}:** {text('integrated', lang)}")
    ...
    with st.container(border=False):
        df = filter_real_data(df, ind, lang)
...
def render_data_methods_page(states, missing_columns, lang):
    ...
    st.markdown(f"### {text('csv_contract', lang)}")
    st.dataframe(dataset_files_df(states, missing_columns, lang), width="stretch", hide_index=True)
...
with st.sidebar:
    ...
    for dataset_name, state in states.items():
        if state == "ok":
            st.success(f"{dataset_name}: {text('status_ok', lang)}")
        elif state == "empty":
            st.warning(f"{dataset_name}: {text('status_empty', lang)}")
        elif state == "bad_schema":
            st.warning(f"{dataset_name}: {text('status_bad_schema', lang)}")
        else:
            st.error(f"{dataset_name}: {text('status_missing', lang)}")

st.title(text("title", lang))
st.caption(text("subtitle", lang))

if page == "schema":
    render_schema_page(states, missing_columns, lang)
    ...
    for indicator in section_indicators:
        with st.container(border=True):
            render_indicator(indicator, population_df, projection_df, unemployment_df, inactivity_df, migration_df, university_df, bes_df, lang)
